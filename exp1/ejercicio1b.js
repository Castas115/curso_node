...
app.get("/producto/:idProducto/cantidad/:cantidad", function(peticion, respuesta) {
    ...
    for(let item of catalogo) {
        if(peticion.params.idProducto == item.id) {
            encontrado = true;
            respuesta.send(`Producto: ${item.nombre}. 
                Precio total: ${item.precio * peticion.params.cantidad}`);
            break;
        }
    }
    if(encontrado == false) {
        respuesta.send("No se ha encontrado el producto.");
    }
